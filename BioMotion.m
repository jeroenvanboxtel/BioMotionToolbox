classdef BioMotion < handle
    ...
    % Manera files can be read
    % CSM files can be read =---  but not check for dropped points yet
    % 
    %
    % BioMotion
    %
    ...
    methods (Access = 'private')
            ...
            for i = 1: nobj
                switch obj(i).Filetype
                    case {'ptd', 'pollick'}
                        ...
                        if length(tline)~=1
                            intermd = load(obj(i).Filename);        % assume this is the Piwek et al (2015) database, which does not start with a number indicating the number of frames
                            intermd = intermd';
                            tline = size(intermd,2);
                        else
        ...
        function loadvanrie(obj)
            ...
            res=regexpi(tline,{'=','[0-9]'});
            inputtype = res{2}(2)-res{2}(1);        % 1 = manera, 2 = vanrie;
            
            res2=tline(sort(cell2mat(res)));
            stringplitEQ = strsplit(res2(2:end),'=');
        ...
        function loadcsm(obj)
            f       = fopen(obj.Filename);
            tline = '';
            interm = [];
            c = 0;
            while isempty(regexp(tline,'Points', 'once'))              
                tline   = fgetl(f);                     %read lines until at start of joints
            end
            while ~feof(f)
                tline   = fgetl(f); 
                readvalues = str2double(strsplit(tline,' '));  
                readvalues = readvalues(2:end-1);
                obj.nPointLights = length(readvalues)/3;
                interm1 = reshape(readvalues,3,obj.nPointLights);
                interm2 = [interm1(1,:) ; interm1(3,:); interm1(2,:)];
                interm = [interm ;interm2];
            end
            if isempty(obj.AnchorJoints)
                obj.AnchorJoints = 1:obj.nPointLights;
            end
            obj.JointsInfo = interm';
